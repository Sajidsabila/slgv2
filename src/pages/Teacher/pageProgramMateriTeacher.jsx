...
import {
  apiGetProgramMateriPublicById,
  apiGetProgramMateriPublic,
} from "../../api/apiPublic";
...
const PageProgramMateriTeacher = () => {
      ...
      {isOpen && (
        <div className="p-0">
          <div className="flex flex-col gap-0">
            {[...new Map(items.map((item) => [item[labelKey], item])).values()].map(
              (item, index) => (
  ...
  return (
    <LandingPageLayout
      title={
        `${replaceTitle2(programById?.class_course ?? "")} ${replaceTitle(programById?.class_grade ?? "")}`.trim() ||
    ...
    >
      <div key={id} className="container mx-auto flex flex-col w-full gap-10 px-4 md:px-10">
        ...
        <div className="flex flex-col md:flex-row gap-7 w-full items-start">
          ...
          <div className="w-full musik flex flex-col w-full  gap-7 py-3 mx-3">
                        ...
                        <div className="mt-3">
                          ...
                          {fileType === "mp4" && (
                            <video
                              ...
                              className="w-80"
                            >
                              <source src={`${import.meta.env.VITE_SISTER_URL}/${item.file_url}`} type="video/mp4" />
                            </video>
                          )}
                          {fileType === "pdf" && (
                            <a
                              href={`${urlLink.url}/${item.file_url}`}
                              target="_blank"
              ...
              <div className="flex justify-center items-center gap-2 mt-4">
                <button
                  ...
                  className={`px-4 py-2 text-sm rounded-md border ${
                    currentPage === 1 ? "text-gray-400 border-gray-300" : "text-slate-700 border-slate-600 hover:bg-slate-200"
                  }`}
                >
                  Previous
                </button>
...
export default PageProgramMateriTeacher;
