...
import {
  apiResourceAdmin,
  apiResourceAdminPost,
  apiResourceAdminDelete,
  apiResourceAdminPut,
} from "../../../api/apiResourceAdmin";
const ProgramMateriSlg = () => {
  ...
  useEffect(() => {
    ...
    const fetchClasssFormat = async () => {
      try {
        const classFormat = await apiResourceAdmin({
          doctype: "Program Class Format",
        });
    ...
    const fetchClassGrading = async () => {
      try {
        const classGrading = await apiResourceAdmin({
          doctype: "Program Class Grading",
        });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const data = {
        type: "SLG",
        class_course:
          courseData.find((c) => c.abbr === selectedCourse)?.name || "",
      };
      const response = isEditMode
        ? await apiResourceAdminPut({
            doctype: "Program Materi",
            name: formData.name,
            id: data,
          })
      ...
      if (!response.name) {
        setError(
          `Data Materi gagal ${isEditMode ? "diperbarui" : "disimpan"}.`,
        );
        return;
      }
      const folderPaths = [
        "Home/Program Materi",
        "Home/Program Materi/SLG",
        `Home/Program Materi/SLG/${response.abbr_course}`,
      ];
      ...
      setSuccess(
        `Data Materi berhasil ${isEditMode ? "diperbarui" : "disimpan"}.`,
      );
      setError(null);
      setProgramMateri((prevData) =>
        isEditMode
          ? prevData.map((item) =>
              item.name === response.name ? response : item,
            )
          : [...prevData, response],
      );
      ...
      console.log(error);
      setError(
        `Error: ${error.response.data.exc_type || error.response.data.exception || error.message}`,
      );
      setSuccess("");
      setIsEditMode(false);
      setIsOpen(false);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setIsLoading(true);
      const deleteResponse = await apiResourceAdminDelete({
        doctype: "Program Materi",
        id,
      });
      ...
      if (deleteResponse) {
        ...
        setProgramMateri((prevData) =>
          prevData.filter((classFormat) => classFormat.name !== id),
        );
      }
    } catch (error) {
      setError(
  ...
  const filteredProgramMateri = programMateri.filter(
    (item) =>
      item.type === "SLG" &&
  ...
  const totalPages =
    Math.ceil(filteredProgramMateri.length / itemsPerPage) || 1;
  ...
  return (
    <AdminLayout>
      <h3 className="font-bold py-7 text-lg">Learning Materi SLG</h3>
      {error && (
        <p className="bg-red-700 text-sm text-white py-3 px-4 my-3">{error}</p>
      )}
      {success && (
        <p className="bg-green-700 text-sm text-white py-3 px-4 my-3">
          {success}
        </p>
      )}
      <div className="flex flex-col w-full p-4 bg-white rounded-xl shadow-lg">
        ...
        {isOpen && !loading && (
          <Modal
            ...
            onClose={handleClose}
            titleModal={
              isEditMode ? "Edit Program Materi" : "Add Program Materi"
            }
                ...
                coursePaginatedData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center flex gap-4">
                      ...
                      <Link
                        to={`/admin/program-materi-SLG/${item.name}`}
...
export default ProgramMateriSlg;
