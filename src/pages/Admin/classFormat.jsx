...
const ClassFormat = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      const courses = await getClassFormat();
  ...
  const filteredClassFormat = courseData.filter((classGrading) =>
    classGrading.name.toLowerCase().includes(search.toLowerCase()) 
  );
  
  const totalPages = Math.ceil(filteredClassFormat.length / itemsPerPage) || 1;
  
  const coursePaginatedData = filteredClassFormat.slice(
